...
p1 <- ggplot(traits, aes(x = (hr.radius))) +
  ...
  scale_color_viridis(discrete = T, option ="D", name = "Class")+
  scale_fill_viridis(discrete = TRUE, option ="D", name = "Class") +
...
p1 <- traits %>% 
  ...
  scale_fill_viridis(discrete = TRUE, option ="D", name ="Medium of\n movement") +
  scale_x_log10(labels = trans_format("log10", math_format(10^.x)))+
  cowplot::theme_cowplot()+
  xlab("Home range radius (km)")
...
p1 <-  traits %>% 
  ...
  geom_histogram(aes(color = diet_broadest_cat, fill = diet_broadest_cat),
                 position = "identity",alpha = 0.2, bins = 30)+
  scale_color_viridis(discrete = T, option ="D", name  ="Trophic level")+
  scale_fill_viridis(discrete = TRUE, option ="D", name  ="Trophic level") +
  scale_x_log10(labels = trans_format("log10", math_format(10^.x)))+
  cowplot::theme_cowplot()+
  xlab("Home range radius (km)")
...
hist.all <- ggarrange(hist.by.class, hist.by.media, hist.by.diet, nrow =3)
ggsave(hist.all, filename = "figure2_12Mar.jpg", dpi = "retina", units = "in", height = 6, width = 10)
...
s3d <- scatterplot3d(x=log10(traits.all.posmig$dispersal_km),
              y=log10(traits.all.posmig$Migration_km +1),
