...
grid_foraging_migrators <-  traits_migrators %>%
  group_by(class) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 320), 
            foraging_year = seq(min(foraging_year, na.rm = TRUE), max(foraging_year, na.rm = TRUE), by = 7))

grid_dispersal_migrators <-  traits_migrators %>%
  group_by(class) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 320), 
            dispersal_year = seq(min(dispersal_year, na.rm = TRUE), max(dispersal_year, na.rm = TRUE), by = 7))

grid_migration_migrators <-  traits_migrators %>%
  group_by(class) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 320), 
            migration_year = seq(min(migration_year, na.rm = TRUE), max(migration_year, na.rm = TRUE), by = 7))
# foraging
means_foraging_migrators <-  grid_foraging_migrators %>%
  add_epred_draws(class_foraging_migrators)

preds_foraging_migrators <-  grid_foraging_migrators %>%
  add_predicted_draws(class_foraging_migrators)

#dispersal
means_dispersal_migrators <-  grid_dispersal_migrators %>%
  add_epred_draws(class_dispersal_migrators)

preds_dispersal_migrators <-  grid_dispersal_migrators %>%
  add_predicted_draws(class_dispersal_migrators)

#migration
means_migration_migrators <-  grid_migration_migrators %>%
  add_epred_draws(class_migration_migrators)

preds_migration_migrators <-  grid_migration_migrators %>%
  add_predicted_draws(class_migration_migrators)


class_for_migrators <- traits_migrators %>%
  ggplot(aes(y = class, x = log10(hr.radius))) +
  stat_interval(aes(x = log10(.prediction)), data = preds_dispersal_migrators[(preds_dispersal_migrators$.prediction>0),]) +
  stat_pointinterval(aes(x = log10(.epred)), data = means_foraging_migrators[(means_foraging_migrators$.epred>0),], .width = c(.66, .95), position = position_nudge(y = -0.3)) +
  geom_point() +
  scale_color_brewer()+
  cowplot::theme_cowplot()+
  labs(y = "Taxonomic class")

class_disp_migrators <- traits_migrators %>%
  ggplot(aes(y = class, x = log10(dispersal_km))) +
  stat_interval(aes(x = log10(.prediction)), 
                data = preds_dispersal_migrators[(preds_dispersal_migrators$.prediction>0),]) +
  stat_pointinterval(aes(x = log10(.epred)), 
                     data = means_dispersal_migrators[(means_dispersal_migrators$.epred>0),], 
...
class_mig_migrators <- traits_migrators %>%
  ggplot(aes(y = class, x = log10(Migration_km))) +
  stat_interval(aes(x = log10(.prediction)), 
                data = preds_migration_migrators[(preds_migration_migrators$.prediction>0),]) +
  stat_pointinterval(aes(x = log10(.epred)), 
                     data = means_migration_migrators[(means_migration_migrators$.epred>0),], 
                     .width = c(.66, .95), position = position_nudge(y = -0.3)) +
  geom_point() +
  scale_color_brewer()+
  cowplot::theme_cowplot()+
...
class_figs_posterior_means <- ggarrange(class_for_migrators, class_disp_migrators, class_mig_migrators, nrow = 1, 
                        common.legend = T, legend = "right", labels = c("a", "b", "c"))


while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/class_figs__migrators_May2023.jpeg", width = 12, height = 3, units = 'in', res = 1000)
