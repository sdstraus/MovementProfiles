...
# library(devtools) # to install ggbiplot from github
...
class_cols <- c(Amphibia = "#00496f", Aves = "#0f85a0", Chondrichthyes = "#edd746", 
                Mammalia = "#ed8b00", Reptilia = "#dd4124")

diet_cols <- c(Carnivore = "#4a3a3b", Herbivore = "#984136", 
                Invertivore = "#c26a7a", Omnivore = "#ecc0a1")

```

The code below is used to create the dummy hypothesis figures presented in Figure 1.
```{r Figure 1}
...
# Figure 2 top row: group by movement medium
# this is only used once, so assigning colors here rather than in chunk 1
media_cols <- c(air = "#fbcc74", land = "#ba783e", water = "#4f412b")


# hist by media
traits$media_foraging[which(traits$media_foraging =="marine")] <- "water"
traits$media_foraging[which(traits$media_foraging =="aquatic")] <- "water"

media_for_distr <- traits %>%
  filter(media_foraging == "air" | media_foraging == "land" |
           media_foraging == "water") %>%
  ggplot(aes(x = media_foraging, y = hr.radius))+
    geom_violin(aes(color = media_foraging, fill = media_foraging))  + 
  geom_boxplot(width = 0.2)+
  scale_color_manual(values = media_cols, name = "Medium")+
  scale_fill_manual(values = media_cols, name = "Medium") +
  scale_y_log10(labels = trans_format("log10", math_format(10^.x)))+
  theme(legend.position = "none")+
  cowplot::theme_cowplot()+
  ylab("Foraging radius (km)")+
  # xlab("Foraging medium")+
  theme(axis.title.x=element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  theme(legend.position = "none")

traits$media_dispersal[which(traits$media_dispersal =="marine")] <- "water"
traits$media_dispersal[which(traits$media_dispersal =="aquatic")] <- "water"

media_disp_distr <- traits %>%
  filter(media_dispersal == "air" | media_dispersal == "land" |
           media_dispersal == "water") %>%
  ggplot(aes(x = media_dispersal, y = dispersal_km))+
    geom_violin(aes(color = media_dispersal, fill = media_dispersal))  + 
  geom_boxplot(width = 0.2)+
  scale_color_manual(values = media_cols, name = "Medium")+
  scale_fill_manual(values = media_cols, name = "Medium") +
  scale_y_log10(labels = trans_format("log10", math_format(10^.x)))+
  ...
  # xlab("Dispersal medium")+
  theme(axis.title.x=element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  theme(legend.position = "none")


traits$media_migration[which(traits$media_migration =="marine")] <- "water"
traits$media_migration[which(traits$media_migration =="aquatic")] <- "water"

media_mig_distr <- traits %>%
  filter(media_migration == "air" | media_migration == "land" |
           media_migration == "water") %>%
ggplot(aes(x = media_migration, y = (Migration_km + 1)))+
    geom_violin(aes(color = media_migration, fill = media_migration))  + 
  geom_boxplot(width = 0.2)+
  scale_color_manual(values = media_cols, name = "Medium")+
  scale_fill_manual(values = media_cols, name = "Medium") +
  scale_y_log10(labels = trans_format("log10", math_format(10^.x)))+
  ...
  # xlab("Migration medium")+
  theme(axis.title.x=element_blank(), axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
  theme(legend.position = "none")


# Figure 2 mid row: group by taxonomic class
...
gB <- ggplotGrob(class_disp_distr)
gC <- ggplotGrob(class_mig_distr)
gD <- ggplotGrob(trophic_for_distr)
gE <- ggplotGrob(trophic_disp_distr)
gF <- ggplotGrob(trophic_mig_distr)

gG <- ggplotGrob(media_for_distr)
gH <- ggplotGrob(media_disp_distr)
gI <- ggplotGrob(media_mig_distr)

gA$heights <- gC$heights
gB$heights <- gC$heights
gD$heights <- gC$heights
gE$heights <- gC$heights
gF$heights <- gC$heights
gG$heights <- gC$heights
gH$heights <- gC$heights
gI$heights <- gC$heights



while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/Figure02_07Aug2023.jpeg", width = 12, height = 11, units = 'in', res = 1000)
grid.arrange(gG, gH, gI, gA, gB, gC, gD,gE, gF, nrow = 3, padding = 2)
dev.off()

```

Create PCA plots for class and trophic guild. We plotted the standardized mass and movement values for three movement types, and also the residual of movement distance ~ body mass for each type. This chode chunk also pulls PCA loadings, presented in the results section and Table S2.
...
while (!is.null(dev.list()))  dev.off()
jpeg(file = "../figures/Figure3_07Aug2023.jpeg", width = 10, height = 10, units = "in", res = 1000)
...
while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/Fig5_epreds_07Aug2023.jpeg", width = 10, height = 10, units = 'in', res = 1000)
grid.arrange(gA, gD, gB, gE,gC, gF, nrow = 3, padding = 2)
dev.off()

```

From here on out, code chunks are used to generate the figures and tables presented in the supplemental materials.

Figure S1. Top panel, birds
```{r Fig S1 birds}
aves <- traits %>% filter(class == "Aves")


aves_dispersal <- readRDS("../mods/aves.disp_withyear.rds")
aves_foraging <- readRDS("../mods/aves.for_withyear.rds")
aves_migration <- readRDS("../mods/aves.mig_withyear.rds")


pred_mod_order_dispersal_aves <- aves %>%
  group_by(order) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 159), 
            dispersal_year = seq(min(dispersal_year), max(dispersal_year), by = 7)) %>% 
  add_epred_draws(aves_dispersal)

pred_mod_order_foraging_aves <- aves %>%
  group_by(order) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 159), 
            foraging_year = seq(min(foraging_year, na.rm = TRUE), max(foraging_year, na.rm = TRUE), by = 7)) %>% 
  add_epred_draws(aves_foraging)

pred_mod_order_migration_aves <- aves %>%
  group_by(order) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 159), 
            migration_year = seq(min(migration_year, na.rm = TRUE), max(migration_year, na.rm=TRUE), by = 7)) %>% 
  add_epred_draws(aves_migration)

aves_foraging_plot <- ggplot() +
  geom_point(data = aves, aes(x = Mass_kg, y = log10(hr.radius), color = ordered(order))) +
  stat_lineribbon(data = pred_mod_order_foraging_aves, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(order), fill = ordered(order)), 
                  .width = c(.95), alpha = 0.4) +
  # scale_color_manual(values = class_cols, name = "Order")+
  # scale_fill_manual(values = class_cols, name = "Order") + 
  scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x),
                labels = scales::trans_format("log10", scales::math_format(10^.x)))+
  scale_y_continuous(labels = scales::math_format())+
  cowplot::theme_cowplot()+
...
aves_dispersal_plot <- ggplot()+
  geom_point(data = aves, aes(x = Mass_kg, y = log10(dispersal_km), color = ordered(order))) +
  stat_lineribbon(data =pred_mod_order_dispersal_aves,
                  aes(y = log10(.epred), x = Mass_kg,
                      color = ordered(order), fill = ordered(order)),
                  .width = 0.95, alpha = 0.4)+
  # scale_color_manual(values = class_cols, name = "Order")+
  # scale_fill_manual(values = class_cols, name = "Order") + 
  scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x),
...
aves_migration_plot <- ggplot() +
  geom_point(data = aves, aes(x = Mass_kg, y = log10(Migration_km), color = ordered(order))) +
  stat_lineribbon(data = pred_mod_order_migration_aves, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(order),fill =ordered(order)), 
                  .width = c(.95), alpha = 0.4) +
  # scale_color_manual(values = class_cols, name = "Order")+
  # scale_fill_manual(values = class_cols, name = "Order") + 
...
gA <- ggplotGrob(aves_foraging_plot)
gB <- ggplotGrob(aves_dispersal_plot)
gC <- ggplotGrob(aves_migration_plot)


while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/aves_order_05May2023.jpeg", width = 16, height = 4, units = 'in', res = 1000)
grid.arrange(gA, gB, gC, nrow = 1, padding = 2)
dev.off()
```


Figure S1. Bottom panel, mammals
```{r Fig S1 mammals}
mam <- traits %>% filter(class == "Mammalia")


mam_dispersal <- readRDS("../mods/mam.disp_withyear.rds")
mam_foraging <- readRDS("../mods/mam.for_withyear.rds")
mam_migration <- readRDS("../mods/mam.mig_withyear.rds")


pred_mod_order_dispersal_mam <- mam %>%
  group_by(order) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 115), 
            dispersal_year = seq(min(dispersal_year), max(dispersal_year), by = 7)) %>% 
  add_epred_draws(mam_dispersal)

pred_mod_order_foraging_mam <- mam %>%
  group_by(order) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 115), 
            foraging_year = seq(min(foraging_year, na.rm = TRUE), max(foraging_year, na.rm = TRUE), by = 7)) %>% 
  add_epred_draws(mam_foraging)

pred_mod_order_migration_mam <- mam %>%
  group_by(order) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 115), 
            migration_year = seq(min(migration_year, na.rm = TRUE), max(migration_year, na.rm=TRUE), by = 7)) %>% 
  add_epred_draws(mam_migration)

mam_foraging_plot <- ggplot() +
  geom_point(data = mam, aes(x = Mass_kg, y = log10(hr.radius), color = ordered(order))) +
  stat_lineribbon(data = pred_mod_order_foraging_mam, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(order), fill = ordered(order)), 
                  .width = c(.95), alpha = 0.4) +
  # scale_color_manual(values = class_cols, name = "Order")+
  # scale_fill_manual(values = class_cols, name = "Order") + 
...
mam_dispersal_plot <- ggplot()+
  geom_point(data = mam, aes(x = Mass_kg, y = log10(dispersal_km), color = ordered(order))) +
  stat_lineribbon(data =pred_mod_order_dispersal_mam,
                  aes(y = log10(.epred), x = Mass_kg,
                      color = ordered(order), fill = ordered(order)),
                  .width = 0.95, alpha = 0.4)+
  # scale_color_manual(values = class_cols, name = "Order")+
...
mam_migration_plot <- ggplot() +
  geom_point(data = mam, aes(x = Mass_kg, y = log10(Migration_km), color = ordered(order))) +
  stat_lineribbon(data = pred_mod_order_migration_mam, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(order),fill =ordered(order)), 
                  .width = c(.95), alpha = 0.4) +
  # scale_color_manual(values = class_cols, name = "Order")+
  # scale_fill_manual(values = class_cols, name = "Order") + 
...
gA <- ggplotGrob(mam_foraging_plot)
gB <- ggplotGrob(mam_dispersal_plot)
gC <- ggplotGrob(mam_migration_plot)


while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/mam_order_05May2023.jpeg", width = 16, height = 4, units = 'in', res = 1000)
grid.arrange(gA, gB, gC, nrow = 1, padding = 2)
dev.off()
```


To create Figure S2, modify the code from Figure 5 above using only migratory species.

```{r Fig S2 migrators only}
traits_migrators <- traits %>% filter(Migration_km > 0)

## class
class_dispersal_migrators <- readRDS("../mods/class.disp_withyear_migrators.rds")
class_foraging_migrators <- readRDS("../mods/class.for_withyear_migrators.rds")
class_migration_migrators <- readRDS("../mods/class.mig_withyear_migrators.rds")


pred_mod_class_dispersal_migrators <- traits_migrators %>%
  group_by(class) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 183), 
            dispersal_year = seq(min(dispersal_year), max(dispersal_year), by = 7)) %>% 
  add_epred_draws(class_dispersal_migrators)

pred_mod_class_foraging_migrators <- traits_migrators %>%
  group_by(class) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 183), 
            foraging_year = seq(min(foraging_year, na.rm = TRUE), max(foraging_year, na.rm = TRUE), by = 7)) %>% 
  add_epred_draws(class_foraging_migrators)

pred_mod_class_migration_migrators <- traits_migrators %>%
  group_by(class) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 183), 
            migration_year = seq(min(migration_year, na.rm = TRUE), max(migration_year, na.rm=TRUE), by = 7)) %>% 
  add_epred_draws(class_migration_migrators)

class_foraging_plot_migrators <- ggplot() +
  geom_point(data = traits_migrators, aes(x = Mass_kg, y = log10(hr.radius), color = ordered(class))) +
  stat_lineribbon(data = pred_mod_class_foraging_migrators, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(class), fill = ordered(class)), 
                  .width = c(.95), alpha = 0.4) +
  scale_color_manual(values = class_cols, name = "Class")+
  scale_fill_manual(values = class_cols, name = "Class") + 
...
class_dispersal_plot_migrators <- ggplot()+
  geom_point(data = traits_migrators, aes(x = Mass_kg, y = log10(dispersal_km), color = ordered(class))) +
  stat_lineribbon(data =pred_mod_class_dispersal_migrators,
                  aes(y = log10(.epred), x = Mass_kg,
                      color = ordered(class), fill = ordered(class)),
                  .width = 0.95, alpha = 0.4)+
  scale_color_manual(values = class_cols, name = "Class")+
...
class_migration_plot_migrators <- ggplot() +
  geom_point(data = traits_migrators, aes(x = Mass_kg, y = log10(Migration_km), color = ordered(class))) +
  stat_lineribbon(data = pred_mod_class_migration_migrators, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(class),fill =ordered(class)), 
                  .width = c(.95), alpha = 0.4) +
  scale_color_manual(values = class_cols, name = "Class")+
  scale_fill_manual(values = class_cols, name = "Class") + 
...
trophic_foraging_migrators <- readRDS("../mods/trophic.for_withyear_migrators.rds")
trophic_dispersal_migrators <- readRDS("../mods/trophic.disp_withyear_migrators.rds")
trophic_migration_migrators <- readRDS("../mods/trophic.mig_withyear_migrators.rds")

pred_mod_trophic_foraging_migrators <- traits_migrators %>%
  group_by(diet_broadest_cat) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 183), 
            foraging_year = seq(min(foraging_year, na.rm = TRUE), max(foraging_year, na.rm = TRUE), by = 7)) %>% 
  add_epred_draws(trophic_foraging_migrators)

pred_mod_trophic_dispersal_migrators <- traits_migrators %>%
  group_by(diet_broadest_cat) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 183), 
            dispersal_year = seq(min(dispersal_year), max(dispersal_year), by = 7)) %>% 
  add_epred_draws(trophic_dispersal_migrators)

pred_mod_trophic_migration_migrators <- traits_migrators %>%
  group_by(diet_broadest_cat) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 183), 
            migration_year = seq(min(migration_year, na.rm = TRUE), max(migration_year, na.rm=TRUE), by = 7)) %>% 
  add_epred_draws(trophic_migration_migrators)

trophic_foraging_plot_migrators <- ggplot() +
  geom_point(data = traits_migrators, aes(x = Mass_kg, y = log10(hr.radius), color = ordered(diet_broadest_cat))) +
  stat_lineribbon(data = pred_mod_trophic_foraging_migrators, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(diet_broadest_cat), fill = ordered(diet_broadest_cat)), 
                  .width = c(.95), alpha = 0.4) +
  scale_color_manual(values = diet_cols, name = "Trophic Guild")+
  scale_fill_manual(values = diet_cols, name = "Trophic Guild") +
...
trophic_dispersal_plot_migrators <- ggplot()+
  geom_point(data = traits_migrators, aes(x = Mass_kg, y = log10(dispersal_km), 
                                color = ordered(diet_broadest_cat))) +
  stat_lineribbon(data = pred_mod_trophic_dispersal_migrators,
                  aes(y = log10(.epred), x = Mass_kg,
                      color = ordered(diet_broadest_cat), 
                      fill = ordered(diet_broadest_cat)),
                  .width = 0.95, alpha = 0.4)+
...
trophic_migration_plot_migrators <- ggplot() +
  geom_point(data = traits_migrators, aes(x = Mass_kg, y = log10(Migration_km), color = ordered(diet_broadest_cat))) +
stat_lineribbon(data = pred_mod_trophic_migration_migrators[(pred_mod_trophic_migration_migrators$.epred>0),],
                  aes(y = log10(.epred), x = Mass_kg, 
                      color = ordered(diet_broadest_cat), 
                      fill = ordered(diet_broadest_cat)), 
                  .width = c(.95), alpha = 0.4) +
...
```{r render S2 figs}
gA <- ggplotGrob(class_foraging_plot_migrators)
gB <- ggplotGrob(class_dispersal_plot_migrators)
gC <- ggplotGrob(class_migration_plot_migrators)
gD <- ggplotGrob(trophic_foraging_plot_migrators)
gE <- ggplotGrob(trophic_dispersal_plot_migrators)
gF <- ggplotGrob(trophic_migration_plot_migrators)
...
while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/FigS2_migrators_07Aug2023.jpeg", width = 10, height = 10, units = 'in', res = 1000)
grid.arrange(gA, gD, gB, gE,gC, gF, nrow = 3, padding = 2)
dev.off()
```
...
```{r Fig S3 non-migrators only}

traits_nonmigrators <- traits %>% filter(Migration_km == 0)

## class
class_dispersal_nonmigrators <- readRDS("../mods/class.disp_withyear_nonmigrators.rds")
class_foraging_nonmigrators <- readRDS("../mods/class.for_withyear_nonmigrators.rds")


pred_mod_class_dispersal_nonmigrators <- traits_nonmigrators %>%
  group_by(class) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 137), 
            dispersal_year = seq(min(dispersal_year), max(dispersal_year), by = 7)) %>% 
  add_epred_draws(class_dispersal_nonmigrators)

pred_mod_class_foraging_nonmigrators <- traits_nonmigrators %>%
  group_by(class) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 137), 
            foraging_year = seq(min(foraging_year, na.rm = TRUE), max(foraging_year, na.rm = TRUE), by = 7)) %>% 
  add_epred_draws(class_foraging_nonmigrators)

class_foraging_plot_nonmigrators <- ggplot() +
  geom_point(data = traits_nonmigrators, aes(x = Mass_kg, y = log10(hr.radius), color = ordered(class))) +
  stat_lineribbon(data = pred_mod_class_foraging_nonmigrators, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(class), fill = ordered(class)), 
                  .width = c(.95), alpha = 0.4) +
  scale_color_manual(values = class_cols, name = "Class")+
  scale_fill_manual(values = class_cols, name = "Class") + 
...
class_dispersal_plot_nonmigrators <- ggplot()+
  geom_point(data = traits_nonmigrators, aes(x = Mass_kg, y = log10(dispersal_km), color = ordered(class))) +
  stat_lineribbon(data =pred_mod_class_dispersal_nonmigrators,
                  aes(y = log10(.epred), x = Mass_kg,
                      color = ordered(class), fill = ordered(class)),
                  .width = 0.95, alpha = 0.4)+
  scale_color_manual(values = class_cols, name = "Class")+
...
trophic_foraging_nonmigrators <- readRDS("../mods/trophic.for_withyear_nonmigrators.rds")
trophic_dispersal_nonmigrators <- readRDS("../mods/trophic.disp_withyear_nonmigrators.rds")

pred_mod_trophic_foraging_nonmigrators <- traits_nonmigrators %>%
  group_by(diet_broadest_cat) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 137), 
            foraging_year = seq(min(foraging_year, na.rm = TRUE), max(foraging_year, na.rm = TRUE), by = 7)) %>% 
  add_epred_draws(trophic_foraging_nonmigrators)

pred_mod_trophic_dispersal_nonmigrators <- traits_nonmigrators %>%
  group_by(diet_broadest_cat) %>%
  data_grid(Mass_kg = seq_range(Mass_kg, n = 137), 
            dispersal_year = seq(min(dispersal_year), max(dispersal_year), by = 7)) %>% 
  add_epred_draws(trophic_dispersal_nonmigrators)


trophic_foraging_plot_nonmigrators <- ggplot() +
  geom_point(data = traits_nonmigrators, aes(x = Mass_kg, y = log10(hr.radius), color = ordered(diet_broadest_cat))) +
  stat_lineribbon(data = pred_mod_trophic_foraging_nonmigrators, 
                  aes(y = log10(.epred), x = Mass_kg, color = ordered(diet_broadest_cat), fill = ordered(diet_broadest_cat)), 
                  .width = c(.95), alpha = 0.4) +
  scale_color_manual(values = diet_cols, name = "Trophic Guild")+
  scale_fill_manual(values = diet_cols, name = "Trophic Guild") +
...
trophic_dispersal_plot_nonmigrators <- ggplot()+
  geom_point(data = traits_nonmigrators, aes(x = Mass_kg, y = log10(dispersal_km), 
                                color = ordered(diet_broadest_cat))) +
  stat_lineribbon(data = pred_mod_trophic_dispersal_nonmigrators,
                  aes(y = log10(.epred), x = Mass_kg,
                      color = ordered(diet_broadest_cat), 
                      fill = ordered(diet_broadest_cat)),
                  .width = 0.95, alpha = 0.4)+
...
gA <- ggplotGrob(class_foraging_plot_nonmigrators)
gB <- ggplotGrob(class_dispersal_plot_nonmigrators)
gC <- ggplotGrob(trophic_foraging_plot_nonmigrators)
gD <- ggplotGrob(trophic_dispersal_plot_nonmigrators)
...
while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/FigS3_nonmigrators_03May2023.jpeg", width = 10, height = 7, units = 'in', res = 1000)
grid.arrange(gA, gC, gB, gD, nrow = 2, padding = 2)
dev.off()
```
...
class_disp <- traits %>%
  ...
  stat_interval(aes(x = log10(.prediction)), 
                data = preds_dispersal[(preds_dispersal$.prediction>0),]) +
  stat_pointinterval(aes(x = log10(.epred)), 
                     data = means_dispersal[(means_dispersal$.epred>0),], 
...
while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/class_figs_posterior_means_May2023.jpeg", width = 12, height = 3, units = 'in', res = 1000)
...
Posterior mean slope distributions, as per reviewer suggestion from EcoLetts, grouped by class.

```{r Fig S5 posterior mean slopes by class}
# class_dispersal <- readRDS("../mods/class.disp_withyear.rds")
# class_foraging <- readRDS("../mods/class.for_withyear.rds")
# class_migration <- readRDS("../mods/class.mig_withyear.rds")

# foraging
coef_no_summary_for <- coef(class_foraging, summary = FALSE)
coef_no_summary_for$class %>% dimnames()
# 1 = iteration, #2 class #3 slopes 
coef_no_summary_for$class[,,] %>% dim()
apply(coef_no_summary_for$class[,,], c(2,3), FUN = function(x) c(mean(x), quantile(x, c(0.025, 0.5,0.975))))

## posterior distribution
class_foraging_slope <- coef_no_summary_for$class[,,2] %>% 
  as.data.frame() %>% 
...
# dispersal
coef_no_summary_disp <- coef(class_dispersal, summary = FALSE)
coef_no_summary_disp$class %>% dimnames()
# 1 = iteration, #2 class #3 slopes 
coef_no_summary_disp$class[,,] %>% dim()
apply(coef_no_summary_disp$class[,,], c(2,3), FUN = function(x) c(mean(x), quantile(x, c(0.025, 0.5,0.975))))

class_dispersal_slope <- coef_no_summary_disp$class[,,2] %>% 
  as.data.frame() %>% 
  pivot_longer(everything(),names_to = "slope", values_to = "posterior_samples") %>% 
  ggplot(aes(x = posterior_samples)) +
  facet_wrap(~slope, ncol = 1) +
...
# migration
coef_no_summary_mig <- coef(class_migration, summary = FALSE)
coef_no_summary_mig$class %>% dimnames()
# 1 = iteration, #2 class #3 slopes 
coef_no_summary_mig$class[,,] %>% dim()
apply(coef_no_summary_mig$class[,,], c(2,3), FUN = function(x) c(mean(x), quantile(x, c(0.025, 0.5,0.975))))

class_migration_slope <- coef_no_summary_mig$class[,,2] %>% 
  as.data.frame() %>% 
  pivot_longer(everything(),names_to = "slope", values_to = "posterior_samples") %>% 
  ggplot(aes(x = posterior_samples)) +
  facet_wrap(~slope, ncol = 1) +
...
Posterior mean slope distributions, as per reviewer suggestion from EcoLetts, grouped by trophic guild.


```{r Fig S6 posterior mean slopes by trophic guild}
# trophic_foraging <- readRDS("../mods/trophic.for_withyear.rds")
# trophic_dispersal <- readRDS("../mods/trophic.disp_withyear.rds")
# trophic_migration <- readRDS("../mods/trophic.mig_withyear.rds")

# foraging
coef_no_summary_for_trophic <- coef(trophic_foraging, summary = FALSE)
coef_no_summary_for_trophic$diet_broadest_cat %>% dimnames()
# 1 = iteration, #2 diet_broadest_cat #3 slopes 
coef_no_summary_for_trophic$diet_broadest_cat[,,] %>% dim()
apply(coef_no_summary_for_trophic$diet_broadest_cat[,,], c(2,3), FUN = function(x) c(mean(x), quantile(x, c(0.025, 0.5,0.975))))

## posterior distribution
trophic_foraging_slope <- coef_no_summary_for_trophic$diet_broadest_cat[,,2] %>% 
  as.data.frame() %>% 
  pivot_longer(everything(),names_to = "slope", values_to = "posterior_samples") %>% 
  ggplot(aes(x = posterior_samples)) +
  facet_wrap(~slope, ncol = 1) +
...
# dispersal
coef_no_summary_disp_trophic <- coef(trophic_dispersal, summary = FALSE)
coef_no_summary_disp_trophic$diet_broadest_cat %>% dimnames()
# 1 = iteration, #2 diet_broadest_cat #3 slopes 
coef_no_summary_disp_trophic$diet_broadest_cat[,,] %>% dim()
apply(coef_no_summary_disp_trophic$diet_broadest_cat[,,], c(2,3), FUN = function(x) c(mean(x), quantile(x, c(0.025, 0.5,0.975))))

trophic_dispersal_slope <- coef_no_summary_disp_trophic$diet_broadest_cat[,,2] %>% 
  as.data.frame() %>% 
  pivot_longer(everything(),names_to = "slope", values_to = "posterior_samples") %>% 
  ggplot(aes(x = posterior_samples)) +
  facet_wrap(~slope, ncol = 1) +
...
# migration
coef_no_summary_mig_trophic <- coef(trophic_migration, summary = FALSE)
coef_no_summary_mig_trophic$diet_broadest_cat %>% dimnames()
# 1 = iteration, #2 diet_broadest_cat #3 slopes 
coef_no_summary_mig_trophic$diet_broadest_cat[,,] %>% dim()
apply(coef_no_summary_mig_trophic$diet_broadest_cat[,,], c(2,3), FUN = function(x) c(mean(x), quantile(x, c(0.025, 0.5,0.975))))

trophic_migration_slope <- coef_no_summary_mig_trophic$diet_broadest_cat[,,2] %>% 
  as.data.frame() %>% 
  pivot_longer(everything(),names_to = "slope", values_to = "posterior_samples") %>% 
  ggplot(aes(x = posterior_samples)) +
  facet_wrap(~slope, ncol = 1) +
...
trophic_post_mean_slopes <- ggarrange(trophic_foraging_slope, trophic_dispersal_slope, trophic_migration_slope, 
          labels = c("Foraging", "Dispersal", "Migration"),
          nrow = 1)

jpeg(filename = "../figures/trophic_posterior_means_slopes.jpeg", width = 9, height = 7, units = 'in', res = 1000)
trophic_post_mean_slopes
dev.off()
...
Code to create covariance matrices and contingency tables, presented in Fig 3c and Tables S1, S3-S4
```{r Covariance matrix and contingency table S1}
class_diet <- table(traits$class, traits$diet_broadest_cat)
...
migration_km1 <- log10(traits$Migration_km+1)
migration <- log10(traits$Migration_km)
mass <- log10(traits$Mass_kg)

cov.mat <- data.frame(home.range, dispersal, migration_km1, mass)
matrix <- cov(cov.mat)
diag(matrix) <- NA
View(matrix)

# remove cases where migration is zero, matrix only for migrators
cov.mat_migrators <- data.frame(home.range, dispersal, migration, mass)
cov.mat2 <- cov.mat_migrators[-c(which(cov.mat_migrators$migration == "-Inf")),]

matrix2 <- cov(cov.mat2, use = "pairwise.complete.obs") 
diag(matrix2) <- NA
View(matrix2)


### non-migrators only ###
cov.mat.nonmig <- cov.mat_migrators %>% filter(migration == "-Inf")
