...
trophic_dispersal_plot <- ggplot()+
  geom_point(data = traits, aes(x = Mass_kg, y = log10(dispersal_km), color = ordered(diet_broadest_cat))) +
  stat_lineribbon(data = pred_mod_trophic_dispersal,
                  aes(y = log10(.epred), x = Mass_kg,
                      color = ordered(diet_broadest_cat), fill = ordered(diet_broadest_cat)),
                  .width = 0.95, alpha = 0.4)+
  scale_color_manual(values = diet_cols, name = "Trophic Guild")+
  scale_fill_manual(values = diet_cols, name = "Trophic Guild") +
  scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x),
...
Figure5 <- ggarrange(class_figs, diet_figs, nrow=3, common.legend=FALSE)

while (!is.null(dev.list()))  dev.off()
jpeg(filename = "../figures/Fig5_epreds_01May2023.jpeg", width = 12, height = 4.5, units = 'in', res = 1000)
Figure5
dev.off()
