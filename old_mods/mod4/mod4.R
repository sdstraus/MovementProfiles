...
mod4_prior$prior[mod4_prior$prior == "student_t(3, 2, 10)"] <- "student_t(3, 2, 5)"

mod4_prior$prior[mod4_prior$prior == "student_t(3, 0, 10)"] <- "student_t(3, 0, 5)"

mod4_prior$prior[mod4_prior$prior == "student_t(3, 1, 10)"] <- "student_t(3, 1, 5)"

mod4 <- brm(
  bf_dispersal + bf_home_range + bf_migration + set_rescor(FALSE), cov_ranef = list(scientific_name.x = A),
  data = traits_phylo, chains = 4, cores = 4, iter =12000,
  control = list(adapt_delta = 0.99, max_treedepth = 15),
  prior = mod4_prior
)

saveRDS(mod4, "mod4_better_factor_as_char.rds")
