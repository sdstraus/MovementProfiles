...
traits_clean <- read.csv("../../data/clean_traits.csv")
...
bf_dispersal <- bf(dispersal_km ~ 1+ (1|scientific_name.x)) + Gamma(link = "log")
bf_home_range <- bf(mean.hra.m2 ~ 1+ (1|scientific_name.x)) + Gamma(link = "log")
bf_migration <- bf(Migration_km ~ 1+ (1|scientific_name.x)) + hurdle_gamma(link = 'log')
...
saveRDS(mod4, "mod4_better_model_des.rds")
