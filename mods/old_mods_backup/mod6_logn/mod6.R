...
bf_dispersal <- bf(dispersal_km ~ log(Mass_kg) + (1|scientific_name.x)) + lognormal(link = "identity")
bf_home_range <- bf(mean.hra.m2 ~ log(Mass_kg) + (1|scientific_name.x)) + lognormal(link = "identity")
bf_migration <- bf(Migration_km ~ log(Mass_kg) + (1|scientific_name.x)) + hurdle_lognormal(link = 'identity')

mod6_prior <- get_prior(bf_dispersal + bf_home_range + bf_migration + set_rescor(FALSE), cov_ranef = list(scientific_name.x = A),
                        data = traits_phylo)

mod6_prior$prior[mod6_prior$class == "Intercept"][2] <- "normal(2,20)"

mod6_prior$prior[mod6_prior$class == "Intercept"][3] <- "normal(11,20)"

mod6_prior$prior[mod6_prior$class == "Intercept"][4] <- "normal(1,20)"

mod6_prior$prior[mod6_prior$prior == "student_t(3, 0, 10)"] <- "student_t(3, 0, 5)"
...
mod6 <- brm(
  ...
  control = list(adapt_delta = 0.995, max_treedepth = 15), save_model = "mod6_s", save_dso = TRUE,  
  file = "mod6_saved"
)
